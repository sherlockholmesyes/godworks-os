...
11. A nested `and` / `or` / `not` `EntityQuery` constraint AST selects the crossed bodies by post-handoff region, spatial box, exact entity-id set, component payload value, and excludes an in-radius decoy with the same broad components.
